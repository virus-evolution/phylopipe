...
include { extract_lineage } from '../modules/preprocess.nf'
...
include { build_tree } from '../modules/build_split_grafted_tree.nf'
...
workflow {
    ...
    if (params.lineage) {
        extract_lineage(mask_and_filter.out.fasta, mask_and_filter.out.metadata)
        ch_masked_fasta = extract_lineage.out.fasta
    } else {
        ch_masked_fasta = mask_and_filter.out.fasta
    }

    if ( params.newick_tree ) {
        ch_tree = Channel.fromPath(params.newick_tree)
        clean_fasta_and_metadata_and_tree(ch_masked_fasta, mask_and_filter.out.metadata, ch_tree)
        ...
        clean_fasta_and_metadata(ch_masked_fasta, mask_and_filter.out.metadata)
    ...
    if ( ! params.protobuf ) {
        if ( ! params.newick_tree and ! params.lineage) {
            ...
            subsample_for_tree(ch_clean_fasta, ch_protected_metadata)
            build_tree(subsample_for_tree.out.fasta, subsample_for_tree.out.metadata, subsample_for_tree.out.hashmap)
            ch_clean_tree = build_tree.out.tree
            ch_fasttree_metadata = subsample_for_tree.out.metadata
        } else {
